...
The QQplot and the histogram clearly support normality. This means it is safe to construct the CI.
...
z = qnorm(0.98)
E = (100/mu)/2
needed_sample_size = (z*z * 0.96 * 0.04)/(E*E)

```

The minimum sample size needed to obtain a CI of 100 grams given the current mean is 550.

```{r bootstrap birthweight}
...
The 96% confidence interval using bootstrap lies between 2806 and 3016. These two confidence intervals are very similar. Normally a bootstrapped CI is constructed because of non-normality or outliers, but this data is distributed normally, so the bootstrap yields the same results as the regular method.

## b)

To investigate the claim by the expert we performed a one-tailed t-test, because the expert claimed the mean birthweight was higher than 2800 grams. The result of this test was that the mean birthweight was indeed higher than 2800. In fact, the lower bound of the 95% CI was 2829 gram. The upper bound was 'Inf' because this was a one-tailed test. The p-value of this t-test was 0.01 which supports H1.
...
The p-value of the t-test is 0.01357, which means the power is 1-0.01357, or approximately 0.986. The p value of the sign test is 0.03399, which means the power is 1-0.03399, or approximately 0.966.
...
There are two options to improve this CI. Firstly, if more data is gathered, the sample size (n) is higher, which leads to a smaller CI and more accuracy at the same confidence level. Secondly, you can use a higher alpha. This will lead to less accuracy at the same confidence level, but will result in a smaller CI.
...
The Shapiro-Wilk test does not reject normality, for the preweight data. As this test does not always reject normality for non-normal distributions, this distribution may still be non-normal (as indicated by the histogram and qqplot).
...
diet_g_boxplt <- ggplot(data, aes( y=weight.lost, group = diet)) + geom_boxplot(color="seagreen", fill="lightgreen") + labs(title='weightloss per diet', x='diet') + theme_light()
diet_g_boxplt
```

The summary shows that the study is not a balanced design. Diet 1 has less participants than diet 2 and 3.
...
Use two-way ANOVA to investigate effect of the diet and gender (and possible interaction) on the lost weight, using the following code: `genderlmi <- lm(weight.lost ~ diet * gender, data = data) ; anova(genderlmi)`, `genderlmm <- lm(weight.lost ~ diet + gender, data = data) ; anova(genderlmm)`.

The two-way ANOVAs below show an interaction effect between gender and diet on weight loss, a main effect of diet on weight loss and no main effect of gender on weight loss. The factor diet is sum parametrized because there is no clear reason to pick one of the diets as a base level.
...
```{r interaction plots gender, echo=FALSE, fig.height=4}
par(mfrow=c(1,2))
interaction.plot(data$gender, data$diet, data$weight.lost,  xlab = 'gender', ylab='weight.lost', trace.label = 'diet')
interaction.plot(data$diet, data$gender, data$weight.lost, xlab = 'diet', ylab= 'weight.lost', trace.label =  'gender')
```
...
The code below makes a dataframe where the soil additives are distributed over the plots, by randomly assigning 2 additives to two plots per block.
...
It appears that, for some blocks, the yield is higher with nitrogen and for other blocks without nitrogen. The mean yield for the plots with nitrogen is higher (57.68 vs 52.07). However, the yield per block varies widely. For some blocks the yield is higher with, and for some without nitrogen. It is therefore important to take block into account, as these results indicate that applying nitrogen will not always lead to a higher yield.

```{r peas npk, echo=FALSE}
npk_N <- npk[npk$N == 1,]
npk_NN <- npk[npk$N == 0,]

N_bxplt <- ggplot(npk_N, aes( y=yield, group = block)) + 
  geom_boxplot(color="seagreen", fill="lightgreen") + labs(title='with nitrogen') + theme_light()
NN_bxplt <- ggplot(npk_NN, aes( y=yield, group = block)) + 
  geom_boxplot(color="seagreen", fill="lightgreen") + labs(title='without nitrogen') + theme_light()
...
The full-two way ANOVA shows no interaction effect between the two, but main effect for block and nitrogen. It was sensible to include block in this model, because, as the previous boxplots already implied, results vary per block. The Friedman test is not applicable in this situation because there are two, not one, observations per cell. Specifically, in each block the condition nitrogen and no nitrogen appears twice.
...
```
First we test for interaction effects
```{r}
anova(npk_lmi)
```
The p-value for interaction effect (block:N) is bigger than 0.05, which means there are no interaction effects. This means we can continue with testing for main effects. In this case we use the model without interaction between the block  and the nitrogen variable.

```{r}
anova(npk_lmm) # main effect for both block and N
```
Both the p-value for the block and the nitrogen variable are lower than 0.05, so we reject h0 and conclude there is a main effect for either the block as the nitrogen on the yield.

## d)

```{r peas d}
model1 = lm(yield ~ N + P + K * block, data = npk)
model2 = lm(yield ~ N + K + P * block, data = npk)
model3 = lm(yield ~ K + P + N * block, data = npk)
model4 = lm(yield ~ N + K + P + block, data = npk)
summary(model1)
summary(model2)
summary(model3)
summary(model4)
```
These models show that there is a significant effect for the use of block 3 and the use of nitrogen. It also shows that there is a significant effect for the use of Potassium in model2, model3, model4, while it doesn't show this in model1. Our favorite model is model3, because it shows the most significant relationships between variables and yield of all the stated effects above.
...
The ANOVA shows that there is a main effect for Nitrogen using a mixed model, with variable block modelled as a random effect.
