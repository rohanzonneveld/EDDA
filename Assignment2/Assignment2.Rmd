...
```
The Shapiro test results in a p-value of 0.011, which indicates the data is not normally distributed, however the qqplot looks fine, so we can assume the data comes from a normal distribution.
...
```
This linear model might not be the best one to explain volume in terms of diameter and height. Since the volume is the product of the surface area and it's height, it would make more sense to construct as follows: volume = pi * (diameter/2)*(diameter/2) * height. In this model the surface area of a circle (tree) is calculated using pi*r*r and it's multiplied by it's height to calculate the volume. The adjusted R-squared of this new model is 0.9743 and the ANOVA between the two submodels and the full model shows that this new model is a better predictor than the old model from C.
