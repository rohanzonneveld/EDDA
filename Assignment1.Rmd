...
## d)
The $1-\alpha$-confidence interval for p is $\hat{p} \pm z_{\alpha/2}\sqrt{\frac{\hat{p}(1-\hat{p})}{n}}$ based on CLT. Since we do not know the true value of p, we can resolve the equation $\hat{p_l} = \hat{p}-z_{\alpha/2}\sqrt{\frac{\hat{p}(1-\hat{p})}{n}}$ and find a value for $\hat{p}$. 
```{r}
...
gender <- c(sample(c(rep("male", 34), rep("female", 28))),sample(c(rep("male", 61), rep("female", 65))))
ascData <- data.frame(weight=order(mydata$birthweight),gender=gender)

p=t.test(weight ~ gender, data = ascData)[[3]]; p
```
Since $p>0.05$, H0 is not rejected which indicates that there is no significant difference in the mean of male and female babies. So, the claim of the expert is false.
...
### Permutation test
The data is paired and we want to test for a difference between groups. Also, a permutation test does not assume any particular distribution of the data, therefore we can apply a permutation test to determine whether the diet has an effect. The R code to perform this test is as follows:
